...
import { CheckCircle2, Search, Download, ChevronLeft, ChevronRight } from 'lucide-react'
...
const inputClass =
  'bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-slate-200 placeholder-slate-500 focus:outline-none focus:border-blue-500 text-sm transition-colors'
...
export default function AdminPage() {
  ...
  const exportCSV = () => {
    const headers = [
      '신청일시', '유입경로', '성함', '연락처', '이메일',
      '투자경험', '관심투자', '막히는부분', '원하는혜택',
      '상태', '쿠폰발송', '메모',
    ]
    const rows = leads.map(l => [
      ...
      l.phone,
      l.email,
      l.investment_experience ?? '',
      l.investment_interest ?? '',
      l.pain_point ?? '',
      (l.desired_benefits ?? []).join(' / '),
      l.status ?? '',
      l.coupon_sent ? 'O' : 'X',
      l.admin_memo ?? '',
    ])
    ...
    <div className="min-h-screen bg-[#090d16] px-4 py-10">
      <div className="max-w-7xl mx-auto">
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-2xl font-black text-slate-100">진단 신청자 DB</h1>
            <p className="text-slate-500 text-sm mt-1">총 {total}명</p>
          </div>
          <button
            onClick={exportCSV}
        ...
        <div className="bg-slate-900/40 border border-slate-800 rounded-xl p-4 mb-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            ...
            <select
              value={filters.investment_experience}
              onChange={e => { setFilters(f => ({ ...f, investment_experience: e.target.value })); setPage(0) }}
              className={`${inputClass} w-full`}
            >
              <option value="">전체 투자경험</option>
              {['아직 시작 전', '계좌는 있지만 거의 안 함', '주식 현물 경험 있음', '주식 선물 경험 있음', '코인 현물 경험 있음', '코인 선물 경험 있음', '현재 손실 중', '꾸준히 매매 중'].map(o => (
                <option key={o} value={o}>{o}</option>
              ))}
            </select>
            <select
              value={filters.investment_interest}
              onChange={e => { setFilters(f => ({ ...f, investment_interest: e.target.value })); setPage(0) }}
              className={`${inputClass} w-full`}
            >
              <option value="">전체 관심투자</option>
              {['주식 현물', '주식 선물', '코인 현물', '코인 선물', '주식과 코인 둘 다', '아직 모르겠음'].map(o => (
                <option key={o} value={o}>{o}</option>
              ))}
            </select>
          </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-800 text-slate-500 text-xs">
                    ...
                    <th className="px-4 py-3 text-left whitespace-nowrap">유입경로</th>
                    <th className="px-4 py-3 text-left whitespace-nowrap">성함</th>
                    <th className="px-4 py-3 text-left whitespace-nowrap">연락처</th>
                    <th className="px-4 py-3 text-left whitespace-nowrap">이메일</th>
                    <th className="px-4 py-3 text-left whitespace-nowrap">투자경험</th>
                    <th className="px-4 py-3 text-left whitespace-nowrap">관심투자</th>
                    <th className="px-4 py-3 text-left whitespace-nowrap">막히는부분</th>
                    <th className="px-4 py-3 text-left whitespace-nowrap">원하는혜택</th>
                    <th className="px-4 py-3 text-left whitespace-nowrap">상태</th>
                    <th className="px-4 py-3 text-center whitespace-nowrap">쿠폰</th>
                    <th className="px-4 py-3 text-left whitespace-nowrap min-w-[160px]">메모</th>
                  </tr>
                </thead>
                <tbody>
                  {leads.map((lead) => (
                    <tr key={lead.id} className="border-b border-slate-800/60 hover:bg-slate-800/20 transition-colors">
                      ...
                      <td className="px-4 py-3 whitespace-nowrap">
                        <span className={`inline-block text-xs px-2 py-0.5 rounded-full font-medium ${
                          lead.source === 'landing_form' ? 'bg-blue-500/10 text-blue-400' :
                          lead.source === 'cta_popup' ? 'bg-purple-500/10 text-purple-400' :
                      ...
                      <td className="px-4 py-3 text-slate-400 text-xs max-w-[160px] truncate">
                        {(lead.desired_benefits ?? []).join(', ') || '-'}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap">
                        <select
                          value={lead.status ?? '신규'}
                      ...
                      <td className="px-4 py-3 text-center">
                        <button
                          onClick={() => updateField(lead.id!, 'coupon_sent', !lead.coupon_sent)}
                      ...
                      <td className="px-4 py-3">
                        <input
                          type="text"
                          value={editMemo[lead.id!] ?? lead.admin_memo ?? ''}
                          onChange={e => setEditMemo(m => ({ ...m, [lead.id!]: e.target.value }))}
                          onBlur={e => {
                            if (e.target.value !== (lead.admin_memo ?? '')) {
                              updateField(lead.id!, 'admin_memo', e.target.value)
                            }
                          }}
                          placeholder="메모..."
                          className="w-full bg-transparent border-b border-slate-700 focus:border-blue-500 text-slate-400 text-xs py-1 focus:outline-none placeholder-slate-600"
                        />
                      </td>
                    </tr>
                  ))}
